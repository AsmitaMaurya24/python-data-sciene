...
def get_news_headlines(url ='https://www.ndtv.com/latest' ):
    ...
    if soup:   
        ...
        result = scrape.extract_many(soup, target =t, items=items, title=title, deet=deet, summary=summary)
        return result
    else:
        print('soup is None')
...
pd.DataFrame(all_data).to_excel('headlines.xlsx') #save your data as excel file